...
public class AccountLoginActivity extends Activity {
	private static final String TAG = AccountLoginActivity.class
			.getSimpleName();
		...
		public void handleMessage(final Message msg) {
			...
			case LOGIN_SUCCESS: {
				Toast.makeText(AccountLoginActivity.this,
						R.string.login_success_label, Toast.LENGTH_SHORT)
						.show();
				EvShareApp.getApplication().setLogined(true);
				if (mAuthenticatorResponse != null) {

					final Account account = new Account(mAccountEditText
							.getText().toString().trim(),
							Authenticator.ACCOUNT_TYPE);
					AccountManager.get(AccountLoginActivity.this)
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mLegalTextView = (TextView) findViewById(R.id.legal_text_view);
...
//		mLegalTextView.setText(legal);
		...
		if (text instanceof Spannable) {
			int end = text.length();
			Spannable sp = (Spannable) mLegalTextView.getText();
			...
			for (URLSpan url : urls) {
				...
				style.setSpan(urlSpan, sp.getSpanStart(url),
...
//		mLegalTextView.getPaint().setAntiAlias(true);
		...
		String account = mSharedPreferences.getString(
				LoginResult.LOGIN_RESULT_ACCOUNT_KEY, null);
		String password = mSharedPreferences.getString(
				LoginResult.LOGIN_RESULT_PASSWORD_KEY, null);

		// if (DEBUG) WLog.d(TAG, "onCreate.account." + account + ", password."
		// + password);
		...
		mAuthenticatorResponse = getIntent().getParcelableExtra(
	...
	private class LoginRunner implements Runnable {
		...
		public void run() {
			...
			try {
				loginResult = WebServiceController.login(mAccountEditText
			...
			if (DEBUG)
				Log.d(TAG, "LoginRunner->" + loginResult);

			// Deal with upload result
			Message msg = new Message();
			if (loginResult != null
					&& TextUtils.isEmpty(loginResult.mLoginResult)) {
				if (DEBUG)
					Log.d(TAG, "LoginRunner->login success");
				Editor e = mSharedPreferences.edit();

				if (mPushCheckBox.isChecked()) {
					// remember account data
					e.putString(LoginResult.LOGIN_RESULT_ACCOUNT_KEY,
							mAccountEditText.getText().toString().trim());
				...
				e.putLong(LoginResult.LOGIN_RESULT_TOTAL_SCORES_KEY,
						loginResult.mTotalScores);
				e.putLong(LoginResult.LOGIN_RESULT_EXPIRY_DATE_KEY,
						loginResult.mExpiryDate);
				e.putString(LoginResult.LOGIN_RESULT_ACCOUNT_DATA_KEY,
						loginResult.mAccountData);

				if (loginResult.mPersonalInfo != null) {
					e.putString(PersonalInfo.PERSONAL_INFO_NAME_KEY,
							loginResult.mPersonalInfo.mName);
					e.putString(PersonalInfo.PERSONAL_INFO_USER_RANK_KEY,
							loginResult.mPersonalInfo.mUserRank);
					e.putString(PersonalInfo.PERSONAL_INFO_ID_TYPE_KEY,
							loginResult.mPersonalInfo.mIdType);
					e.putString(PersonalInfo.PERSONAL_INFO_ID_KEY,
							loginResult.mPersonalInfo.mId);
					e.putString(PersonalInfo.PERSONAL_INFO_PHONE_NUMBER_KEY,
							loginResult.mPersonalInfo.mPhoneNumber);
					e.putString(PersonalInfo.PERSONAL_INFO_EMAIL_ADDRESS_KEY,
							loginResult.mPersonalInfo.mEmailAddress);
				}
				...
				if (loginResult != null) {
					...
					if (DEBUG)
						Log.d(TAG, "LoginRunner->login failure."
